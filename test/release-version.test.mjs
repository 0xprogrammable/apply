...
test("every release version projection fails closed when changed independently", (t) => {
  const cases = [
    ["package.json", (value) => { value.version = "1.5.1"; }, "RELEASE_VERSION_MISMATCH"],
    ["package-lock.json", (value) => { value.version = "1.5.1"; }, "RELEASE_VERSION_MISMATCH"],
    ["package-lock.json", (value) => { value.packages[""].version = "1.5.1"; }, "RELEASE_VERSION_MISMATCH"],
    ["registry/config.json", (value) => { value.historyVersion = "1.5.1"; }, "RELEASE_VERSION_MISMATCH"],
    ["registry/config.json", (value) => { value.updatedAt = "2026-08-14T00:00:01Z"; }, "RELEASE_HISTORY_MISMATCH"],
    ["registry/history/1.5.0.json", (value) => { value.version = "1.5.1"; }, "RELEASE_HISTORY_MISMATCH"],
    ["registry/history/1.5.0.json", (value) => { value.generatedAt = "2026-08-14T00:00:01Z"; }, "RELEASE_HISTORY_MISMATCH"]
  ];
...
test("released history snapshots are append-only and byte-immutable", (t) => {
  ...
  fs.writeFileSync(path.join(extra, "registry/history/1.5.1.json"), "{}\n");
  assert.throws(
