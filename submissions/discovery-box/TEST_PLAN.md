...
Use native ETH as `currency0`. Test zero, 1, 999, 1,000, 999,000, 1,000,000, `type(int128).max` boundaries and randomized amounts. Zero execution is a no-op; every positive gross quote below 1,000 smallest units must revert atomically under policy 1.1.
...
The displayed formulas are the zero-carry identities. The implementation must maintain policy 1.1 cumulative lifetime accounting across accepted swaps. Test split accepted swaps in each mode and an alternating-mode sequence; claims must not reset remainder state, and positive gross quote amounts below 1,000 smallest units must revert instead of accumulating dust.
...
- every positive gross quote amount below 1,000 smallest units reverts without changing liability, claims or remainder state
...
- deterministic Builder 1.5 preflight: completed with `DESIGN_REVIEW_REQUIRED` and fee integration pending
- 20,000-path reduced-form economic model: passed
- prior-policy Solidity build, unit, fuzz, invariant and static-analysis evidence: recorded; policy 1.1 minimum-volume repair and rerun required
- fork evidence: planned
