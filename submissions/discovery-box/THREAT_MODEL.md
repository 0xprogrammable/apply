...
The target fee is always native ETH and 10 basis points of executed gross quote-side volume. Exact-output gross-up uses denominator 999,000. Policy 1.1 requires cumulative lifetime accounting and atomically rejects positive gross quote amounts below 1,000 smallest units. The current source carries fractional fee remainders across all four quadrants but does not yet enforce that minimum, so the fee integration remains pending and must not be treated as prototype-conformant.
...
| Sub-minimum quote bypass | Reject every positive gross quote below 1,000 smallest units before changing claims, liability or remainder state |
...
- Policy 1.1 minimum-volume enforcement is not yet implemented; fee integration remains pending.
