...
| Programmable fee | Target policy: exactly 0.10% of executed gross ETH-side volume on every canonical-pool swap. The project takes no hook-owned fee. The current source still needs the policy 1.1 minimum-volume guard before this integration is complete. |
...
The formulas above show the zero-carry case. The current source carries the fractional remainder between swaps in a common `999,000,000` denominator (the least common multiple of the gross and fee-on-top denominators). Policy 1.1 additionally requires every positive gross quote amount below 1,000 smallest units to revert atomically. That guard is not yet implemented, so `programmableFee.collection.status` remains `pending-hook-integration` and this application does not claim prototype conformance.
...
- Local evidence: the prior policy-version test suite passed across token behaviour, all 4 swap modes, fee claims, partial fills and the first buy from a BOX-only pool. It does not prove policy 1.1 conformance.
- Not yet evidenced: fork execution, deployed addresses, live fee collection, routing support and product availability.

## Open decisions

There are no remaining owner-level architecture decisions for the isolated prototype. The implementation must add the policy 1.1 minimum-volume guard and matching tests before fee integration can be marked implemented. Exact deployment records, runtime hashes, initial price, tick range and production app commitments remain release inputs. Changing the 40-box target or a fee path requires a new preflight revision.
