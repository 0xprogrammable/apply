...
- Use executed quote on after-swap paths; prove both specified-quote before-swap paths either fill the fee-adjusted amount exactly or revert the parent swap, then reconcile the platform, 10% VRF reserve, 90% reward split, PoolManager final deltas, raw ETH, liabilities, and operating reserve.
