...
The buyer list is append-only and has no fixed wallet cap. Ordinary dust transfers cannot grow it; each new entry needs at least `0.005 ETH` of retained canonical-buy basis. A qualifying canonical buy never fails because a draw group is full. Each round commits to at most 512 wallets in fixed circular queue order. Each finalization call scans at most 256 entries and stores only its group, phase, cursor, weights, and three provisional winners. One weight pass and three winner-selection passes require at most 8 maximum-size calls. Timed-out recovery needs at most 4. An attacker can spend capital and fees to grow the stored registry and delay later queue positions, but cannot increase one round's scan work. A completed group pass with fewer than three eligible buyers leaves the pot intact, advances the queue, and emits failure.
