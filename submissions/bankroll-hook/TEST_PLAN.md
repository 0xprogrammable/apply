...
`test/fork/BankrollEthereum.fork.t.sol` runs at Ethereum block 25,690,000. It checks the exact PoolManager, PositionManager, UERC20Factory, WETH and Chainlink wrapper runtimes and interfaces, then completes the full token, pool, reviewed hook/router and permanently locked position lifecycle against those contracts. Set `MAINNET_RPC_URL` to an archive RPC and run `npm run test:fork`; the normal `npm test` command excludes fork tests explicitly so CI cannot silently depend on an RPC.
